...
class Loss(nn.modules.loss._Loss):
    def __init__(self, args):
        ...
        for loss in args.loss.split('+'):
            ...
            if loss_type == 'MSE':
                ...
                module = import_module('ST_MFNet.losses.charbonnier')
                loss_function = getattr(module, 'Charbonnier')()
            elif loss_type == 'Lap':
                module = import_module('ST_MFNet.losses.laplacianpyramid')
                loss_function = getattr(module, 'LaplacianLoss')()
            elif loss_type.find('GAN') >= 0:
                module = import_module('ST_MFNet.losses.adversarial')
    ...
    def forward(self, output, gt, input_frames):
        ...
        for l in self.loss:
            if l['function'] is not None:
                if l['type'] in ['FI_GAN', 'FI_Cond_GAN', 'STGAN', 'T_WGAN_GP']:
