...
def get_flow(im1, im2):
    ...
    if np.any((im1 < 0) | (im1 > 1)):
        print("out of range break")
        print(im1)
        sys.exit()
    if np.any((im2 < 0) | (im2 > 1)):
        print("out of range break")
        print(im2)
        sys.exit()
...
class Vimeo90k_septuplet(data.Dataset):
    def __init__(self, db_dir, train=True, upscale_factor=4, transform=None, crop_sz=(256,256)):
    ...
    def __getitem__(self, index):
        ...
        if self.crop_sz is not None:
            #rawFrame1, rawFrame2, rawFrame3, rawFrame4, rawFrame5, rawFrame6, rawFrame7 = vt.rand_crop(rawFrame1, rawFrame2, rawFrame3, rawFrame4, rawFrame5, rawFrame6, rawFrame7, sz=self.crop_sz)
            rawFrame1, rawFrame2, rawFrame3, rawFrame4, rawFrame5, rawFrame6, rawFrame7 = vt.center_crop(rawFrame1, rawFrame2, rawFrame3, rawFrame4, rawFrame5, rawFrame6, rawFrame7, sz=self.crop_sz)
        target = modcrop(rawFrame4, self.upscale_factor)

        input = target.resize((int(target.size[0]/self.upscale_factor),int(target.size[1]/self.upscale_factor)), Image.BICUBIC)

        neigbor = [rawFrame1, rawFrame2, rawFrame3, rawFrame5, rawFrame6, rawFrame7]
