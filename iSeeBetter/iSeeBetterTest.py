...
from data import get_test_set, transform
...
#parser.add_argument('-m', '--model', default="weights/netG_epoch_4_1_APITLoss.pth", help="Model")
...
test_set = Vimeo90k_septuplet(db_dir=args.data_dir, train=False, upscale_factor=args.upscale_factor, transform=transform())
...
def main():
    ...
    loop = tqdm(testing_data_loader, leave=True)
    for idx, batch in enumerate(loop):
        ...
        for metric in metrics:
            if metric in ['PSNR' ,'SSIM']:
                scores = getattr(utility, 'calc_{}'.format(metric.lower()))(target, prediction)
                for score in scores:
                    tmp_dict[metric].append(score.item())
        for i in range(len(prediction)):
            imwrite(prediction[i], os.path.join(args.output, "pd_psnr_{}_ssim_{}.png".format(tmp_dict['PSNR'][idx*args.testBatchSize+i], tmp_dict['SSIM'][idx*args.testBatchSize+i])), range=(0, 1))
            imwrite(target[i], os.path.join(args.output, "gt_psnr_{}_ssim_{}.png".format(tmp_dict['PSNR'][idx*args.testBatchSize+i], tmp_dict['SSIM'][idx*args.testBatchSize+i])), range=(0, 1))
    ...
    with open(os.path.join(args.output, file_name), "w") as json_file:
